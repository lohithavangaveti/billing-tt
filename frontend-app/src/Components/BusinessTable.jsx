  ...
  const saveEdit = async (id) => {
    if (api && loadData) {
      ...
      const updated = businesses.map((b) =>
        b.id === id ? { ...b, name: editName, category: editCategory } : b
      );
            ...
            <tr key={b.id} style={{ textAlign: "center" }}>
              ...
              {editId === b.id ? (
                ...
                <>
                  ...
                  <td style={td}>
                    <button onClick={() => { setEditId(b.id); setEditName(b.name); setEditCategory(b.category); }}>
                      Edit
                    </button>
                    <button onClick={() => deleteBusiness(b.id)} style={{ marginLeft: "5px" }}>
                      Delete
                    </button>
                    <button onClick={() => handleBilling(b)} style={{ marginLeft: "5px" }}>
                      Billing
                    </button>
                  </td>
                </>
